...
tol_new=1e-5 #tolerance for iteration
...
def hist(u,p):
    return 0.5*inner(sigma(u),epsilon(u))+dot(u,grad(p))
# def hist(u,p):
#     return 0.5*lambda_*(tr(epsilon(u)))**0.5+mu*tr(epsilon(u)*epsilon(u))+dot(u,grad(p))

# def hist(u,p):
#     return 0.5*(lambda_(Nu)+2/3*mu(Nu))*(0.5*(tr(epsilon(u))+abs(tr(epsilon(u)))))**2+mu(Nu)*inner(dev(epsilon(u)),dev(epsilon(u)))+dot(u,grad(p))
def H(unew,p,Hold):
...
# F2=inner(g(d_i,tol)*(sigma(u)-p*Identity(len(u))),grad(v))*dx-p*dot(grad(g(d_i,tol)),v)*dx
...
ufile_pvd = File("poro_fluid_5/displacement.pvd")
pfile_pvd = File("poro_fluid_5/pressure.pvd")
crack_pvd = File ("poro_fluid_5/crack.pvd")
